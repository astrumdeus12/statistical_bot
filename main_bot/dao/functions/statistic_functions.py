...
async def get_first_last_days(days):
...
async def count_all_sells(first_day, last_day):
    async for session in get_async_session():
        async with session.begin():
            users_sells = await session.execute(
                select(
                ...
                ).where(Sells.day >= first_day)
                .where(Sells.day <= last_day))
            
            result = users_sells.fetchone()
            result_dict = {}
            if result:
                result_dict.update({k: getattr(result, k) for k in result._mapping})
            else:
                result_dict = {'total_credits': 0, 'total_insurance': 0, 'box_insurance': 0, 'total_client_calls':0, 'total_deb_cards': 0, 'total_credit_cards': 0, 'total_investition_insurance': 0, 'period_credit_count': 0, 'total_credit_count': 0}
                
            result_answer = f'''\n Продажи за указанный период: \n Кредиты: {result_dict['total_credits'] or 0} \n Количество кредитов: {result_dict['total_credit_count']} \n Страховки: {result_dict['total_insurance'] or 0} \n Коробочные страхования: {result_dict['box_insurance']} \n Звонки: {result_dict["total_client_calls"] or 0} \n Дебетовые карты: {result_dict['total_deb_cards'] or 0} \n Кредитные карты: {result_dict["total_credit_cards"] or 0} \n Инвестиционное страхование жизни: {result_dict["total_investition_insurance"] or 0}'''
...
async def get_user_sells_statistic(days:int, user_tg_id:str):
    async for session in get_async_session():
        async with session.begin():
            days_list = await get_first_last_days(days)
...
async def get_all_sells_statistic(days:int):
    async for session in get_async_session():
        async with session.begin():
            ...
            result_answer = await count_all_sells(first_day, last_day)
